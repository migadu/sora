...
package managesieve_connection_limits
...
// setupManageSieveServerWithConnectionLimits creates a ManageSieve server with specific connection limits
func setupManageSieveServerWithConnectionLimits(t *testing.T, maxTotal, maxPerIP int) (*managesieve.ManageSieveServer, string) {
	t.Helper()
	...
	// Create ManageSieve server with connection limits
	sieveServer, err := managesieve.New(
		context.Background(),
		"test",
		"localhost",
		address,
	...
	go func() {
		sieveServer.Start(errChan)
	}()

	// Check for immediate startup errors
	select {
	case err := <-errChan:
		t.Fatalf("ManageSieve server failed to start: %v", err)
	...
	return sieveServer, address
}

func TestManageSieveConnectionLimiterTotal(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)

	// Set up server with 2 total connections
	server, address := setupManageSieveServerWithConnectionLimits(t, 2, 10)
	defer server.Close()
...
func TestManageSieveConnectionLimiterPerIP(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
