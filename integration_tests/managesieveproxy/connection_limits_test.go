...
// TestManageSieveProxyPerIPConnectionLimiting tests that ManageSieve proxy properly limits
// connections per IP based on trusted_networks configuration
func TestManageSieveProxyPerIPConnectionLimiting(t *testing.T) {
	...
	// Set up ManageSieve proxy with connection limits
	...
	t.Log("=== Testing ManageSieve Proxy Per-IP Connection Limiting ===")
	...
	conn1, err := dialManageSieveProxy(proxyAddress)
	if err != nil {
	...
	conn2, err := dialManageSieveProxy(proxyAddress)
	if err != nil {
	...
	conn3, err := dialManageSieveProxy(proxyAddress)
	if err != nil {
		...
		isAlive := testManageSieveConnectionAlive(conn3)
		if !isAlive {
...
// setupManageSieveProxyWithConnectionLimits creates a ManageSieve proxy with connection limiting
func setupManageSieveProxyWithConnectionLimits(t *testing.T, rdb *resilient.ResilientDatabase,
	proxyAddr string, backendAddrs []string, maxConnections, maxConnectionsPerIP int,
	trustedNetworks []string) *common.TestServer {
	t.Helper()

	hostname := "test-managesieve-proxy-limits"
	masterUsername := "proxyuser"
	masterPassword := "proxypass"

	opts := managesieveproxy.ServerOptions{
		Name:                   "test-managesieve-proxy-limits",
		Addr:                   proxyAddr,
		RemoteAddrs:            backendAddrs,
		RemotePort:             4190, // Default ManageSieve port
	...
	if err != nil {
		t.Fatalf("Failed to create ManageSieve proxy with limits: %v", err)
	}
...
// dialManageSieveProxy connects to ManageSieve proxy for testing
func dialManageSieveProxy(address string) (interface{ Close() error }, error) {
	return dialManageSieveProxyTimeout(address, 2*time.Second)
}

// dialManageSieveProxyTimeout connects to ManageSieve proxy with timeout
func dialManageSieveProxyTimeout(address string, timeout time.Duration) (interface{ Close() error }, error) {
...
// testManageSieveConnectionAlive tests if a ManageSieve connection is still alive
func testManageSieveConnectionAlive(conn interface{ Close() error }) bool {
