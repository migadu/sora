...
// TestManageSieveProxyConnectionLimiterCounters tests that connection limiter counters
...
func TestManageSieveProxyConnectionLimiterCounters(t *testing.T) {
	...
	// Set up ManageSieve proxy with specific limits for testing
	proxyAddress := common.GetRandomAddress(t)
	proxyServer := setupManageSieveProxyWithConnectionLimits(t, backendServer.ResilientDB, proxyAddress,
		[]string{backendServer.Address}, 10, 3, []string{}) // maxTotal=10, maxPerIP=3
	defer proxyServer.Close()

	// Get the limiter to verify counter state
	proxy, ok := proxyServer.Server.(*managesieveproxy.Server)
	if !ok {
		t.Fatalf("Failed to cast to ManageSieve proxy server")
	}
	...
	t.Log("=== Testing ManageSieve Proxy Connection Limiter Counters ===")
...
// TestManageSieveProxyConnectionLimiterEnforcement tests that connection limits are properly enforced
func TestManageSieveProxyConnectionLimiterEnforcement(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
	...
	// Set up ManageSieve proxy with very restrictive limits
	proxyAddress := common.GetRandomAddress(t)
	proxyServer := setupManageSieveProxyWithConnectionLimits(t, backendServer.ResilientDB, proxyAddress,
	...
	t.Log("=== Testing ManageSieve Proxy Connection Limiter Enforcement ===")
