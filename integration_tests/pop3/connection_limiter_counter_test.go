...
// TestPOP3BackendConnectionLimiterCounters tests that connection limiter counters
...
func TestPOP3BackendConnectionLimiterCounters(t *testing.T) {
	...
	server, ok := backendServer.Server.(*pop3.POP3Server)
	if !ok {
		t.Fatalf("Failed to cast to POP3 server")
	}
	...
	t.Log("=== Testing POP3 Backend Connection Limiter Counters ===")
