...
// startIMAPWithMasterUsername starts an IMAP backend configured with the
...
func startIMAPWithMasterUsername(t *testing.T, rdb *resilient.ResilientDatabase, rateLimit server.AuthRateLimiterConfig, masterUser, masterPass string) string {
	t.Helper()
	...
	srv, err := serverImap.New(
		...
		serverImap.IMAPServerOptions{
			...
			MasterUsername: []byte(masterUser),
...
// handled by DIFFERENT code (login.go and sasl.go), each deriving its own
// rate-limit key, so both must be probed.
var masterUsernameVerbs = []struct {
	name  string
	probe func(t *testing.T, address, username, password string) string
}{
	{"LOGIN", func(t *testing.T, address, username, password string) string {
		t.Helper()
		return imapRawCommand(t, address, fmt.Sprintf("LOGIN %q %q", username, password))
	}},
	{"AUTHENTICATE_PLAIN", func(t *testing.T, address, username, password string) string {
		t.Helper()
		return imapAuthPlain(t, address, "", username, password)
	}},
}
...
// TestIMAP_MasterUsername_FailuresDoNotLockOutImpersonationTarget proves that a
...
func TestIMAP_MasterUsername_FailuresDoNotLockOutImpersonationTarget(t *testing.T) {
	...
	for _, verb := range masterUsernameVerbs {
		t.Run(verb.name, func(t *testing.T) {
			address := startIMAPWithMasterUsername(t, rdb, masterUsernameRateLimit(), masterUser, masterPass)
			...
			resp := imapRawCommand(t, address, fmt.Sprintf("LOGIN %q %q", account.Email, account.Password))
			if !strings.HasPrefix(resp, "A001 OK") {
...
// TestIMAP_MasterUsername_BruteForceIsMeteredAcrossTargets proves the other half
...
func TestIMAP_MasterUsername_BruteForceIsMeteredAcrossTargets(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
	...
	for _, verb := range masterUsernameVerbs {
		t.Run(verb.name, func(t *testing.T) {
			...
			// broken.
			control := startIMAPWithMasterUsername(t, rdb, masterUsernameRateLimit(), masterUser, masterPass)
			if resp := verb.probe(t, control, account.Email+"@"+masterUser, masterPass); !strings.HasPrefix(resp, "A001 OK") {
				t.Fatalf("control: master username login must work, got %q", resp)
			}

			probed := startIMAPWithMasterUsername(t, rdb, masterUsernameRateLimit(), masterUser, masterPass)
			...
			// ONE bucket.
			if resp := verb.probe(t, probed, account.Email+"@"+masterUser, masterPass); strings.HasPrefix(resp, "A001 OK") {
