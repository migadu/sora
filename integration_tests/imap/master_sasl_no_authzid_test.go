...
// startIMAPWithMasterSASL starts an IMAP backend with the given master SASL
...
func startIMAPWithMasterSASL(t *testing.T, rdb *resilient.ResilientDatabase, rateLimit server.AuthRateLimiterConfig, saslUser, saslPass string) string {
	t.Helper()
...
// imapRawCommand runs one command on a fresh connection and returns the verbatim
// tagged response line. Raw sockets rather than imapclient: the assertion is on
// the bytes the server puts on the wire.
func imapRawCommand(t *testing.T, address, command string) string {
	t.Helper()
	...
	if _, err := fmt.Fprintf(conn, "A001 %s\r\n", command); err != nil {
...
// imapAuthPlain performs one AUTHENTICATE PLAIN exchange (SASL-IR) and returns
// the verbatim tagged response line.
func imapAuthPlain(t *testing.T, address, authzID, authnID, password string) string {
	t.Helper()

	encoded := base64.StdEncoding.EncodeToString([]byte(authzID + "\x00" + authnID + "\x00" + password))
	return imapRawCommand(t, address, "AUTHENTICATE PLAIN "+encoded)
}

// TestIMAP_MasterSASL_NoAuthzIDIsNotAPasswordOracle proves that with an EMPTY
...
// identity (target user to impersonate).") let anyone who can reach the port
// confirm a guessed master password without ever completing an authentication.
//
...
func TestIMAP_MasterSASL_NoAuthzIDIsNotAPasswordOracle(t *testing.T) {
	...
	for _, shape := range shapes {
		t.Run(shape.name, func(t *testing.T) {
			...
			// rate-limiter state.
			correctAddr := startIMAPWithMasterSASL(t, rdb, rateLimit, shape.saslUser, saslPass)
			wrongAddr := startIMAPWithMasterSASL(t, rdb, rateLimit, shape.saslUser, saslPass)

			// Enough attempts to cross the Tier-1 threshold (3) on both sides.
			const attempts = 5

			for i := 0; i < attempts; i++ {
				withMaster := imapAuthPlain(t, correctAddr, "", shape.saslUser, saslPass)
				withWrong := imapAuthPlain(t, wrongAddr, "", shape.saslUser, fmt.Sprintf("wrong-master-pw-%d", i))

				if strings.HasPrefix(withMaster, "A001 OK") {
...
// TestIMAP_MasterSASL_NoAuthzIDCorrectPasswordIsRecorded proves that presenting
...
func TestIMAP_MasterSASL_NoAuthzIDCorrectPasswordIsRecorded(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
	...
	control := startIMAPWithMasterSASL(t, rdb, rateLimit, saslUser, saslPass)
	if resp := imapAuthPlain(t, control, account.Email, saslUser, saslPass); !strings.HasPrefix(resp, "A001 OK") {
		t.Fatalf("control: master SASL impersonation must work, got %q", resp)
	}

	probed := startIMAPWithMasterSASL(t, rdb, rateLimit, saslUser, saslPass)

	// Burn the Tier-1 budget with the CORRECT master password and no authzid.
	for i := 0; i < 3; i++ {
		if resp := imapAuthPlain(t, probed, "", saslUser, saslPass); strings.HasPrefix(resp, "A001 OK") {
	...
	if resp := imapAuthPlain(t, probed, account.Email, saslUser, saslPass); strings.HasPrefix(resp, "A001 OK") {
...
// TestIMAP_MasterSASLUsernameIsARealAccount covers the deployment where
// master_sasl_username is also a real account's address (e.g. an operator's own
// mailbox). That account must keep working with its OWN password, over LOGIN and
// over SASL PLAIN, and the proxy's impersonation of it must keep working too.
func TestIMAP_MasterSASLUsernameIsARealAccount(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
	...
	address := startIMAPWithMasterSASL(t, rdb, server.AuthRateLimiterConfig{Enabled: false}, saslUser, saslPass)

	// LOGIN with the account's own password.
	if resp := imapRawCommand(t, address, fmt.Sprintf("LOGIN %q %q", account.Email, account.Password)); !strings.HasPrefix(resp, "A001 OK") {
		t.Fatalf("account whose address is master_sasl_username cannot log in with its own password over LOGIN: %q", resp)
	}

	// AUTHENTICATE PLAIN with an empty authorization identity and the account's own password.
	if resp := imapAuthPlain(t, address, "", account.Email, account.Password); !strings.HasPrefix(resp, "A001 OK") {
	...
	// AUTHENTICATE PLAIN with authzid == authcid and the account's OWN password. RFC 4616
	// says an authorization identity equal to the authentication identity means "no
	// impersonation", and several SASL client libraries fill the field in unconditionally,
	// so this is an ordinary login that happens to name itself. Answering it from the
	// master-SASL branch - where the password does not match the master password - locks
	// this account out of its own mailbox permanently.
	if resp := imapAuthPlain(t, address, account.Email, account.Email, account.Password); !strings.HasPrefix(resp, "A001 OK") {
	...
	// The proxy flow must survive: imapproxy sends authzid=<user>, authcid=<master
	// SASL username>, which for this account are the same string.
	if resp := imapAuthPlain(t, address, account.Email, saslUser, saslPass); !strings.HasPrefix(resp, "A001 OK") {
	...
	if resp := imapAuthPlain(t, address, "", saslUser, saslPass); strings.HasPrefix(resp, "A001 OK") {
