...
func TestIMAP_ESearchOptions(t *testing.T) {
	...
	t.Log("Running ESEARCH RETURN (ALL)")
	esearchOpts = &imap.SearchOptions{
		ReturnAll: true,
	}
	res, err = c.Search(&imap.SearchCriteria{}, esearchOpts).Wait()
	...
	// Note: res.All.Nums() since we changed things, let's just check length
	var seqSet []uint32
	if ss, ok := res.All.(imap.SeqSet); ok {
		seqSet, _ = ss.Nums()
	}
	if len(seqSet) != 3 {
		t.Errorf("Expected 3 seq nums, got %d", len(seqSet))
	}
	...
	if len(seqSet2) != 3 {
		t.Errorf("ALL mismatch: %d", len(seqSet2))
	}
