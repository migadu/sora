...
// TestIMAPBackendConnectionLimiterCounters tests that connection limiter counters
...
func TestIMAPBackendConnectionLimiterCounters(t *testing.T) {
	...
	server, ok := backendServer.Server.(*imap.IMAPServer)
	if !ok {
		t.Fatalf("Failed to cast to IMAP server")
	}
	...
	t.Log("=== Testing IMAP Backend Connection Limiter Counters ===")
