...
// TestIMAP_MasterSASL_BadAuthzIDIsNotAPasswordOracle proves that a REJECTED
// impersonation target does not tell the caller whether the master password was
// right.
//
// A reply reachable only with the correct master password ("Impersonation target
// user address is not in the correct format.", "Impersonation target user account
// not found.") confirms a guessed tenant-wide credential without ever completing
// an authentication — the same class as the empty-authzid oracle already closed.
//
...
func TestIMAP_MasterSASL_BadAuthzIDIsNotAPasswordOracle(t *testing.T) {
	...
	for _, target := range targets {
		t.Run(target.name, func(t *testing.T) {
			...
			// rate-limiter state.
			correctAddr := startIMAPWithMasterSASL(t, rdb, rateLimit, saslUser, saslPass)
			wrongAddr := startIMAPWithMasterSASL(t, rdb, rateLimit, saslUser, saslPass)

			// Enough attempts to cross the Tier-1 threshold (3) on both sides.
			const attempts = 5

			for i := 0; i < attempts; i++ {
				withMaster := imapAuthPlain(t, correctAddr, target.authzID, saslUser, saslPass)
				withWrong := imapAuthPlain(t, wrongAddr, target.authzID, saslUser, fmt.Sprintf("wrong-master-pw-%d", i))
...
// TestIMAP_MasterSASL_RejectedAuthzIDConsumesMasterBudget proves the accounting
...
func TestIMAP_MasterSASL_RejectedAuthzIDConsumesMasterBudget(t *testing.T) {
	common.SkipIfDatabaseUnavailable(t)
	...
	for _, target := range targets {
		t.Run(target.name, func(t *testing.T) {
			...
			for i := 0; i < 3; i++ {
				if resp := imapAuthPlain(t, probed, target.authzID, saslUser, saslPass); strings.HasPrefix(resp, "A001 OK") {
