...
// TestManageSieveBackendConnectionLimiterCounters tests that connection limiter counters
...
func TestManageSieveBackendConnectionLimiterCounters(t *testing.T) {
	...
	server, ok := backendServer.Server.(*managesieve.ManageSieveServer)
	if !ok {
		t.Fatalf("Failed to cast to ManageSieve server")
	}
	...
	t.Log("=== Testing ManageSieve Backend Connection Limiter Counters ===")
	...
	// Note: localhost may be in trusted networks, so per-IP tracking might not apply
	t.Logf("✓ Per-IP count for localhost: %d (may be 0 if localhost is trusted)", localhostCount)
