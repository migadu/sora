...
// TestIMAPProxySimpleConnectionLimit tests basic connection limiting without authentication
func TestIMAPProxySimpleConnectionLimit(t *testing.T) {
	...
	// Set up IMAP proxy with maxPerIP=1 (very restrictive for testing)
	...
	t.Log("=== Testing Simple IMAP Proxy Connection Limiting ===")
	t.Logf("Proxy config: maxTotal=5, maxPerIP=1, trustedNetworks=[]")
	t.Log("Testing TCP connections only (no IMAP authentication)")
