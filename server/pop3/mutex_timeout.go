...
func (s *POP3Session) acquireReadLockWithTimeout() (bool, context.CancelFunc) {
	...
	case <-ctx.Done():
		...
		go func() {
			...
			case <-time.After(time.Second):
				...
				s.Log("[POP3][MUTEX] Critical warning: Failed to clean up read lock after timeout")
			}
		}()
		return false, cancel
	}
...
func (s *POP3Session) acquireWriteLockWithTimeout() (bool, context.CancelFunc) {
	ctx, cancel := context.WithTimeout(s.ctx, MutexTimeout)
	lockChan := make(chan struct{})
	...
	case <-ctx.Done():
		...
		go func() {
			...
			case <-time.After(time.Second):
				// If we can't get the lock after waiting another second,
				// something is seriously wrong, but we can't do much about it
				s.Log("[POP3][MUTEX] Critical warning: Failed to clean up write lock after timeout")
			}
		}()
		return false, cancel
	}
