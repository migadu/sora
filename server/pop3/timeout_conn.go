...
// wrapWithTimeout wraps an existing connection with timeout enforcement
// This is used when we need to wrap a connection that's already been accepted
func wrapWithTimeout(conn net.Conn, timeout time.Duration, absoluteTimeout time.Duration, minBytesPerMinute int64, protocol string) net.Conn {
	if timeout > 0 || absoluteTimeout > 0 || minBytesPerMinute > 0 {
		// Check if already wrapped
		if _, ok := conn.(*timeoutConn); ok {
			return conn
		}
		return newTimeoutConn(conn, timeout, absoluteTimeout, minBytesPerMinute, protocol)
	}
	return conn
}
