		...
		if err != nil {
			...
			case proxy.AuthUserNotFound:
				...
				if s.server.prelookupConfig != nil && s.server.prelookupConfig.FallbackDefault {
					...
					if s.server.debug {
						log.Printf("IMAP Proxy [%s] User '%s' not found in prelookup. Fallback enabled - attempting main DB authentication.", s.server.name, username)
					}
				} else {
					// Fallback disabled - reject immediately
					log.Printf("IMAP Proxy [%s] User '%s' not found in prelookup. Fallback disabled (fallback_to_default=false) - rejecting authentication.", s.server.name, username)
					s.server.authLimiter.RecordAuthAttemptWithProxy(s.ctx, s.clientConn, nil, username, false)
					metrics.AuthenticationAttempts.WithLabelValues("imap_proxy", "failure").Inc()
...
func (s *Session) registerConnection() error {
	...
	if s.server.connTracker != nil && s.server.connTracker.IsEnabled() {
		return s.server.connTracker.RegisterConnection(ctx, s.accountID, "IMAP", clientAddr, s.serverAddr, s.username, true)
	}
	return nil
}
