...
func (rd *ResilientDatabase) RegisterConnectionWithRetry(ctx context.Context, accountID int64, protocol, clientAddr, serverAddr, instanceID, email string, isProxy bool) error {
	...
	op := func(ctx context.Context, tx pgx.Tx) (interface{}, error) {
		return nil, rd.getOperationalDatabaseForOperation(true).RegisterConnection(ctx, tx, accountID, protocol, clientAddr, serverAddr, instanceID, email, isProxy)
	}
