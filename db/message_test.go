...
func TestCopyMessages(t *testing.T) {
	...
	emptyUIDs := []imap.UID{}
	uidMapping, _, err := db.CopyMessages(ctx, tx2, &emptyUIDs, mailboxID, sentMailbox.ID, accountID, "example.com", "user", "")
	assert.NoError(t, err)
	...
	uidMapping, _, err = db.CopyMessages(ctx, tx3, &nonExistentUIDs, mailboxID, sentMailbox.ID, accountID, "example.com", "user", "")
	assert.NoError(t, err)
