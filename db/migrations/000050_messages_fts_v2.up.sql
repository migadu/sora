...
--   (shared mailboxes included) because that is what messages.account_id always holds.
--
...
-- (every statement below is IF NOT EXISTS). Full runbook in
-- tasks/fts-per-account-composite-gin.md. Short form, all on the primary, direct (not via a
-- transaction pooler):
--
...
--      catch-up query until it reports 0 rows twice.
