...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_subject_trgm_partial ON messages USING gin (LOWER(subject) gin_trgm_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_recipients_json_partial ON messages USING GIN (recipients_json jsonb_path_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_from_email_sort_trgm_partial ON messages USING gin (from_email_sort gin_trgm_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_from_name_sort_trgm_partial ON messages USING gin (from_name_sort gin_trgm_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_to_email_sort_trgm_partial ON messages USING gin (to_email_sort gin_trgm_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_to_name_sort_trgm_partial ON messages USING gin (to_name_sort gin_trgm_ops) WHERE expunged_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_cc_email_sort_trgm_partial ON messages USING gin (cc_email_sort gin_trgm_ops) WHERE expunged_at IS NULL;

-- 2. Drop the old full indexes concurrently
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_subject_trgm;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_recipients_json;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_from_email_sort_trgm;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_from_name_sort_trgm;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_to_email_sort_trgm;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_to_name_sort_trgm;
DROP INDEX CONCURRENTLY IF EXISTS idx_messages_cc_email_sort_trgm;
